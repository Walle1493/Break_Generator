...
import sys
sys.path.append('../')
from utils import Seq2SeqDataset, ProtoQA_Dataset
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    if not evaluate:
        ...
        type_path = 'dev'
    dataset = Seq2SeqDataset(tokenizer, data_dir=args.data_dir, type_path=type_path, max_source_length=args.max_src_len, max_target_length=args.max_tgt_len)
    return dataset
...
def model_step(batch: dict, model, tokenizer):
    ...
    if 'labels' in batch:
        ...
        if isinstance(model, BartForConditionalGeneration):
            ...
            # else isinstance(model, T5ForConditionalGeneration):
            decoder_input_ids = target_ids[:, :-1].contiguous()
            lm_labels = target_ids[:, 1:].clone()
            for i in range(len(lm_labels)):
                for j in range(len(lm_labels[i])):
...
def save_model(model, location, T5_split=-1):
    ...
    if T5_split != -1:
        count = T5_split
        assert model.config.num_layers % count == 0
        device_num = model.config.num_layers // count
        device_map = dict()
        for i in range(count):
            device_map[i] = [j for j in range(i*device_num, (i+1)*device_num)]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        logger.info(f'current step: {global_step}')
                        results = evaluate(args, model, tokenizer)
                        for key, value in results.items():
                            tb_writer.add_scalar('eval_{}'.format(key), value, global_step)
                        if results['eval_loss'] < best_loss:
                            logger.info("Saving model checkpoint to %s", args.output_dir)
                            save_model(model, args.output_dir, args.T5_split)
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    save_model(model, output_dir, args.T5_split)
...
def evaluate(args, model, tokenizer, prefix="", dynamics=False):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_output_dir = args.output_dir
    if args.switch_dataset:
        eval_dataset = ProtoQA_Dataset(args.data_dir, args.model_type, args.max_src_len, args.max_tgt_len, tokenizer, 'dev', evaluate=False, experiment=args.experiment)
    else:
...
def main():
    ...
    parser.add_argument("--switch_dataset", action='store_true')
    ...
    parser.add_argument("--T5_split", type=int, default=-1)
    ...
    if args.do_train:
        ...
        os.system("cp run_fine_tuning.py %s" % os.path.join(args.output_dir, 'run_fine_tuning.py'))
        os.system("cp run_generation.py %s" % os.path.join(args.output_dir, 'run_fine_generation.py'))
    ...
    if args.T5_split != -1:
        count = args.T5_split
        assert config.num_layers % count == 0
        device_num = config.num_layers // count
        device_map = dict()
        for i in range(count):
            device_map[i] = [j for j in range(i*device_num, (i+1)*device_num)]
        model.parallelize(device_map)            
    ...
    # add #1-#6 tokens to bart
    # if args.model_type == 'bart':
    #     index_tokens = ["#1", "#2", "#3", "#4", "#5", "#6"]
    #     num_added_toks = tokenizer.add_tokens(index_tokens)
    #     print('We have added', num_added_toks, 'tokens')
    ...
    if args.switch_dataset:
        train_dataset = ProtoQA_Dataset(args.data_dir, args.model_type, args.max_src_len, args.max_tgt_len, tokenizer, 'train', evaluate=False, experiment=args.experiment)
    else:
